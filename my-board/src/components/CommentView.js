...
const selectBoard = (setBoard, name) => {
  axios
    .get("http://localhost:8080/board/" + name)
    .then((res) => setBoard(res.data));
};

const selectComment = (board, setComments) => {
  axios.get("http://localhost:8080/comment/" + board.boardNo).then((res) => {
...
const updateComment = (comment, contentValue) => {
  axios
    .put("http://localhost:8080/comment/" + comment.commentNo, {
...
const deleteComment = (comment, setCheck) => {
  axios
    .delete("http://localhost:8080/comment/" + comment.commentNo)
    .then((res) => {
      alert("삭제완료");
      setCheck(true);
    });
